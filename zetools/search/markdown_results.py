...
def insert_markdown_results_section(results: List['MarkdownFile'], results_page: 'MarkdownFile') -> 'MarkdownFile':
    """Builds the results section of the page."""
    results_section = _results_table_header.format(num_results=len(results))
    for result in results:
        results_section = results_section + _result_template.format(section_title=result.title,
                                                                    filepath=result.rel_filepath)
